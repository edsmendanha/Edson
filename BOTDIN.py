...
# -*- coding: utf-8 -*-
# =============================================================================
# BOTDIN.py — Multi-ativo calibrado M5/M1 (2026-03-28)
# =============================================================================
# Arquivo: BOTDIN.py  |  Raiz do repositório  |  Usa Ativos.txt e config.txt
#
# FUNCIONALIDADES:
#   ✅ Multi-ativo dinâmico — até 4 ativos simultâneos
#   ✅ Leitura do Ativos.txt: prioridade digital, fallback binária automático
#   ✅ OTC só opera se o usuário pedir no menu — nunca mistura OTC com OP
#   ✅ Sincronização exata nas viradas de vela (primeiros 2s do candle novo)
#   ✅ Filtros calibrados por timeframe:
#        M5 — ADX≥10, BB≥0.00060, SLOPE≥0.00007, ATR≥0.00015  (moderado)
#        M1 — ADX≥4,  BB≥0.00018, SLOPE≥0.00003, ATR≥0.00005  (permissivo, 2-de-4)
#   ✅ Score V15 por timeframe: M5=40 mínimo, M1=28 mínimo
#   ✅ Cooldown inteligente: 2 losses seguidos → pausa 20min só naquele ativo
#   ✅ Prints detalhados: sinal, score, mercado, timing, resultado
#   ✅ Reconexão automática ao detectar queda de conexão
#   ✅ Logs CSV: trades, latência, padrões, sincronização
#
# CALIBRAÇÃO FÁCIL (bloco "ESTRATÉGIA" abaixo):
#   Para mais entradas no M5:   reduza ADX_MIN_M5, BB_WIDTH_MIN_M5 ou V15_SCORE_MIN_M5
#   Para mais entradas no M1:   reduza ADX_MIN_M1 ou V15_SCORE_MIN_M1
#   Para menos entradas (mais seletivo): aumente os valores acima
#   Para ajustar cooldown:      ASSET_LOSS_COOLDOWN_COUNT / ASSET_LOSS_COOLDOWN_SECONDS
#   Para sincronização:         CANDLE_SYNC_LEAD_SECONDS / CANDLE_SYNC_GRACE_SECONDS
#
# DEPENDÊNCIAS (já usadas no repositório):
#   pip install configobj iqoptionapi
#
# Testado em demo com até 4 ativos simultâneos no M1 e M5.
# =============================================================================
...
BOTDIN_VERSION = "BOTDIN-2026-03-28-v1"
...
# =====================================================================
# ESTRATÉGIA: BOTDIN — FILTROS CALIBRADOS M5/M1
# =====================================================================
# Parâmetros centralizados aqui. Altere SOMENTE neste bloco.
#
# M5 (moderado): filtros ATR + ADX + BBW + SLOPE todos habilitados e com
#   valores reais que evitam operar em compressão/lateralidade intensa,
#   mas sem travar de vez. Score mínimo V15=40 (mais seletivo).
#
# M1 (permissivo): regra "2-de-4" — permite até 2 filtros abaixo do mínimo
#   entre ATR, ADX, BBW e SLOPE. Mínimos baixos mas não zero, garantindo
#   algum critério. Score mínimo V15=28 (mais entradas).
#
# Para mais entradas M5:  reduza ADX_MIN_M5, BB_WIDTH_MIN_M5 ou V15_SCORE_MIN_M5
# Para mais entradas M1:  reduza ADX_MIN_M1 ou V15_SCORE_MIN_M1
# Para menos entradas:    aumente os valores acima
# =====================================================================

ENABLE_ATR_FILTER = True       # ← BOTDIN: filtro ATR habilitado (M5 exige, M1 usa 2-de-4)
ATR_PERIOD = 14
...
ATR_MIN_RATIO_ABS_M1 = 0.00005    # ← BOTDIN: mínimo leve para M1 (não zero)
ATR_MIN_RATIO_ABS_M5 = 0.00015    # ← BOTDIN: mínimo moderado para M5 (calibrado)
ATR_RATIO_QUEUE_M1 = deque(maxlen=ATR_ADAPTIVE_WINDOW)
ATR_RATIO_QUEUE_M5 = deque(maxlen=ATR_ADAPTIVE_WINDOW)

ENABLE_TREND_STRENGTH_FILTER = True   # ← BOTDIN: filtros ADX/BBW/SLOPE habilitados (M5 rígido, M1 via 2-de-4)
ADX_PERIOD = 14
ADX_MIN_M1 = 4.0                  # ← BOTDIN: mínimo leve para M1 (permissivo, 2-de-4)
ADX_MIN_M5 = 10.0                 # ← BOTDIN: mínimo moderado para M5 (calibrado)
BB_PERIOD = 20
BB_STD = 2.0
BB_WIDTH_MIN_M1 = 0.00018         # ← BOTDIN: aceita leve compressão M1, não zero
BB_WIDTH_MIN_M5 = 0.00060         # ← BOTDIN: mínimo moderado para M5 (calibrado)
SLOPE_LOOKBACK = 8
SLOPE_MIN_M1 = 0.00003            # ← BOTDIN: mínimo leve para M1
SLOPE_MIN_M5 = 0.00007            # ← BOTDIN: mínimo moderado para M5 (calibrado)
...
# BOTDIN: scores mínimos calibrados por timeframe (M5 mais seletivo, M1 mais permissivo)
# ← AJUSTE: aumente V15_SCORE_MIN_M5/M1 para mais seletividade, reduza para mais entradas
V15_SCORE_MIN = 35           # ← BOTDIN: score mínimo padrão (0–100; usado como fallback)
V15_SCORE_MIN_M5 = 40        # ← BOTDIN: score mínimo para M5 (mais seletivo)
V15_SCORE_MIN_M1 = 28        # ← BOTDIN: score mínimo para M1 (mais permissivo)
...
def check_patterns(tf_min: int, velas: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    # ← BOTDIN: seleciona score mínimo conforme o timeframe via lookup
    _V15_SCORE_BY_TF = {1: V15_SCORE_MIN_M1, 5: V15_SCORE_MIN_M5}
    _score_min = _V15_SCORE_BY_TF.get(tf_min, V15_SCORE_MIN)
    if call_score >= _score_min and (call_score - put_score) >= V15_SCORE_GAP_MIN:
    ...
    if put_score >= _score_min and (put_score - call_score) >= V15_SCORE_GAP_MIN:
        # ── Filtro estrutural M5 (v15.1): só aceita sinal no extremo do range ──
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 70)
    print(f"🤖 BOTDIN — Multi-ativo M5/M1  |  v{BOTDIN_VERSION}")
    print("=" * 70)
